...
# "/Users/scuerda/Work/Projects/_DATAPROCESSING/catalog/Common"
# Get geography object for CT and subcounty divisions
ctGeos <- getCTGeos("town")
# years for which we will process this dataset
# yearList = c(2013, 2012, 2011, 2010)
yearList = c(2015) # only getting new data from raw files

ltm <- proc.time()[1:3]
# table is set in outer loop (see race_ethnicity_loop.R)
tn = table
acsdata <- getACSData(ctGeos, yearList = yearList, table = tn)
ltm <- proc.time()[1:3] - ltm
for(time in names(ltm)) {
    print(paste(time, " load time for table ", tn, " year ", 2010, ": ", ltm[time], " seconds"))
}
...
for(data in acsdata) {
    ...
    # percent.total <- divide.acs(total, total, method = "proportion")
    # acs.colnames(percent.total) <- "Percent:Total:Total"
...
towns <- fread("town_fips.csv", colClasses = "character")
setkey(towns, FIPS)
dataset <- towns[dataset]
remove(towns)

#set final column order
setcolorder(dataset, c(1,2,3,8,6,9,4,5,7))

ptm <- proc.time()[1:3] - ptm
for(time in names(ptm)) {
    print(paste(time, " process time for table ", tn, ", all years ", ": ", ptm[time], " seconds"))
}
